...
gs_tools.load_crafts = function(item)
	...
	if minetest.get_modpath("technic") then
		...
		-- recipes
		local ttypes = { "alloy", "grinding", "compressing", "extracting" }
		for _, tname in pairs(ttypes) do
			for _, r in pairs(technic.recipes[tname].recipes) do
				local t = {}
				t.type = string.sub(tname, 1, 5)
				if ItemStack(r.output):get_name() == item then
					t.width = 0
					t.items = {}
					t.count = {}
					for n, v in pairs(r.input) do
						table.insert(t.items, n)
			...
			for k=1,9 do
				...
				if v and #v > 0 then
					...
					if not s2:is_empty() then
						...
						if not s2:is_empty() and p:contains_item("main", s2) then
...
gs_tools.workbench_formspec = function(pos, params)
	...
	-- player inventory
	local pinv = 
		"list[current_player;main;4.5,4.75;" .. gs_tools.player_inv_width .. ",1;]" ..
		"list[current_player;main;4.5,5.98;" .. gs_tools.player_inv_width .. ",3;" .. 
			gs_tools.player_inv_width .. "]" ..
		default.get_hotbar_bg(4.5,4.75)
	...
	local fs = "size[" .. fw .. ",8.75]" ..
		default.gui_bg .. default.gui_bg_img .. default.gui_slots ..
		mods .. listhead .. ilv .. stxt .. rec ..
		cbut .. craft .. pinv
